...
stock = "LT.NS" 
# Entering a sample stock from Nifty 50 rather than prompitng user to enter a stock ticker
#input("Enter a stock ticker: ")
...
# Fetching the stock data from yfinance library
yf.pdr_override()
...
# Starting from the 60th day till the 80% duration of historical data,
# Create sequences of 60 consecutive 'Close' prices (x_train) and their corresponding next day's 'Close' price (y_train). 
# Each iteration of the loop will append a sequence of the previous 60 days as input (x_train) 
# and the next day's 'Close' price as the target output (y_train).
...
# LSTM network: use two LSTM layers each having 50 units; followed by two dense layers
model = Sequential([
...
#similar iteration over a period of 60 days 
...
# Visualize the data
# check the predicted prices against the historical prices
